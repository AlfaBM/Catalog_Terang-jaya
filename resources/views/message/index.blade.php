        ...
        <table class="table table-light table-hover" style="table-layout:fixed">
            <tbody>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <button type="submit" class="btn btn-danger" onclick="return confirm('Ingin menghapus data?')" disabled>Delete</button>
                    <div>
                        <form action="{{ route('message.index') }}" method="POST">
                            @csrf
                            @method('GET')
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" placeholder="Search Message..." name="search" value="{{ request('search') }}">
                ...
                <tr onmouseover="" style="cursor: pointer;" data-href="{{ route('message.show', $mes['id']) }}">
                    <td style="width:5vw"><input type="checkbox" name="cek[{{ $mes['id'] }}]" value="{{ $mes['id'] }}"></td>
                    <td style="width:10px;">
                        @if ($mes['read'])
                    ...
                    <td style="width:15vw; white-space: nowrap; overflow: hidden;
