...
<div class="col-6 m-auto mt-3">
    <form action="{{ Route('Catalog.admin.update', $catalog['id']) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3 text-start">
            ...
            <input class="form-control " type="text" name="nama" id="nama" value="{{ $catalog['nama'] }}" required autocomplete="off">
        </div>
        <div class="mb-3 text-start">
            <label for="deskripsi">Detail Deskripsi</label>
            <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi" rows="3" placeholder="Max 500 Character (huruf)" required autocomplete="off">{{ $catalog['deskripsi'] }}</textarea>
            @error('deskripsi')
        ...
        <div class="mb-3 text-start">
            ...
            <input class="form-control" value="{{ $catalog['produsen'] }}" type="text" name="produsen" id="produsen" required>
        </div>
        <div class="mb-3 text-start">
            <label for="ukuran">Ukuran</label>
            <input class="form-control @error('ukuran') is-invalid @enderror" value="{{ $catalog['ukuran'] }}" type="text" name="ukuran" id="ukuran" placeholder="" required>
            @error('ukuran')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-3 text-start">
            ...
            <input class="form-control @error('harga') is-invalid @enderror" value="{{ 'RP.'.number_format(floatval($catalog['harga']), 0, ',', '.') }}" type="text" name="harga" id="harga" required>
            @error('harga')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-3 text-start">
            ...
            @if ($catalog['gambar'])
            <img id="gambarold" name="gambarold" src="{{ asset('storage/' . $catalog['gambar']) }}" widht="130px" height="60ox" alt="">
            @else
        ...
        <div class="mb-3 text-start">
            <label for="gambarnew">Update Gambar <small style="color:rgb(0, 0 ,0 , .4)">*jika ingin
                    update</small></label>
            <input class="form-control @error('gambarnew') is-invalid @enderror" type="file" name="gambarnew" id="formfile">
            @error('gambarnew')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
